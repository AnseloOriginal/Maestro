...
export async function filesInFolderInformation(foldername,namefilter="") {
  const request = await createFileSytem("request")
  const folder = await request.get("folders", foldername) //Get folder information
  if (folder) {
    const groupOfFiles = []
    for (let i=0;i<folder.files.length;i++) {
      const filename = folder.files[i];
      const fileObject = await request.get("files",filename) //Might be undefined [TODO: Find undefined]
...
export async function moveFile(source,dest,filename) {
  ...
  const request = await createFileSytem("request")
  const souceFolder = await folderInformation(source)//Gets the folder
  const file = await filesInFolderInformation(source,filename) //Gets the file
